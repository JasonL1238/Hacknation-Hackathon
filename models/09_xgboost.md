...
the committed Mash split, grouped train-only tuning, `scale_pos_weight`, and normalized
inverse labeled `dedup_group_id` sample weights. Retain it as the strongest current
nonlinear reference; do not retune from test.
...
Retain XGBoost because it was the strongest held-out weighted-Brier model for several
antibiotics in the completed run. Keep HistGradientBoosting as the dependency-light tree
reference, and do not add more tree libraries unless they answer a distinct question.
...
- Pass normalized sample_weight = 1 / labeled dedup-group size in every fit and calibration step, and use those weights for duplicate-weighted tuning and reporting.
