...
Prefer HistGradientBoosting as the default boosting choice when you want strong tabular performance without adding XGBoost/LightGBM/CatBoost as dependencies, or when reproducibility and a single environment matter more than squeezing the last fraction of AUROC. It is the lowest-friction step up from the Random Forest baseline. Skip it, or compare against, when you specifically need CatBoost's ordered boosting for very small data, LightGBM's leaf-wise speed, or XGBoost's SHAP tooling and GPU path — but always confirm those extras actually improve the per-genetic-group Brier before accepting the added complexity.
...
- **Imbalance:** use per-sample weights (inverse class frequency) via `sample_weight`; do not use SMOTE across the grouped structure.
...
- Handle class imbalance with sample_weight (inverse class frequency). Do NOT use SMOTE/synthetic oversampling.
